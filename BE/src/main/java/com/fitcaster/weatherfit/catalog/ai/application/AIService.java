// AIService.java
// AI 관련 서비스

package com.fitcaster.weatherfit.catalog.ai.application;

import com.fitcaster.weatherfit.catalog.ai.api.dto.AIRequestDTO;
import com.fitcaster.weatherfit.catalog.ai.api.dto.AIResponseDTO;
import com.fitcaster.weatherfit.common.exception.InternalServerException;
import org.springframework.ai.chat.client.ChatClient;
import org.springframework.ai.openai.OpenAiChatOptions;
import org.springframework.core.io.Resource;
import org.springframework.stereotype.Service;
import org.springframework.util.MimeType;
import java.util.List;
import java.util.regex.Pattern;
import java.util.regex.Matcher;

// * author: 김기성
@Service
public class AIService {

    private final ChatClient chatClient;

    // ChatClient 주입 생성자
    public AIService(ChatClient.Builder chatClientBuilder) {
        this.chatClient = chatClientBuilder.build();
    }

    // AI 설명 텍스트에서 최고/최저 기온 파싱
    public AIResponseDTO.TemperatureInfo parseTemperatureFromDescription(String aiDescription) {
        if (aiDescription == null || aiDescription.isEmpty()) {
            return null;
        }

        Integer minTemperature = null;
        Integer maxTemperature = null;

        // "이 옷에 가장 적절한 최고/최저 기온은 OO°C / XX°C입니다." 형식에서 기온 파싱 (음수 포함)
        Pattern pattern = Pattern.compile("최고/최저 기온은 (-?\\d+)°C / (-?\\d+)°C입니다.");
        Matcher matcher = pattern.matcher(aiDescription);
        if (matcher.find()) {
            try {
                maxTemperature = Integer.parseInt(matcher.group(1));
                minTemperature = Integer.parseInt(matcher.group(2));
            } catch (NumberFormatException e) {
                // 파싱 실패 시 로그 기록
                System.err.println("Failed to parse temperatures: " + e.getMessage());
            }
        }

        return new AIResponseDTO.TemperatureInfo(minTemperature, maxTemperature);
    }

    // 상품 정보/이미지 기반으로 AI 설명 생성
    public AIResponseDTO generateDescription(AIRequestDTO request) {
        try {
            // 시스템 프롬프트 (AI 역할&출력 형식 정의)
            String systemPrompt = """
                    [역할]
                     - 당신은 '웨더핏(WeatherFit)' 소속의 전문 패션 컨설턴트이자 날씨 기반 의류 추천 전문가입니다.
                     - 당신의 주된 임무는 제공된 '상품명, 분류, 카테고리, 성별, 추천 계절'에 대한 상품 정보와 상품 이미지를 분석하여, 사용자가 구매 결정을 내리는 데 도움이 되는 실용적인 스타일 가이드를 작성합니다.\s
                    
                    [작업 목표 (Goal)]
                     - 상품 정보와 이미지를 종합적으로 분석하여, 총 5가지 항목으로 구성된 상세하고 매력적인 AI 상품 설명을 생성합니다. 이 설명은 모든 방문객에게 공통으로 제공되므로, 다양한 체질의 사용자들이 모두 유용한 정보를 얻어갈 수 있도록 작성해야 합니다.
                     - 당신은 날씨와 구매자들의 다양한 체질과 상황을 고려한 실용적이고 구체적인 가이드를 작성하여, 구매자가 옷을 구매하고 실제 날씨에 맞춰 입는 데 도움이 되는 심층적인 AI 설명을 생성해야 합니다.
                    
                    [입력으로 제공되는 정보]
                    - 상품명, 분류(아우터/상의/하의 등), 카테고리(니트, 코트, 데님 등), 성별, 추천 계절, 상품 이미지가 전달될 수 있습니다.
                    
                    [작성 언어/톤]
                    - 반드시 한국어로만 작성합니다.
                    - 영어 또는 한자 등의 다른 언어는 사용하지 않고 작성합니다.
                    - 말투는 "~입니다/합니다"의 공손하지만 딱딱하지 않은 설명형으로 유지합니다.
                    - AI 모델이라는 사실은 절대 언급하지 않습니다.
                    
                    [출력 형식(Format) 공통 규칙]
                     - 결과는 반드시 "일반 텍스트"로만 작성하고, 마크다운 문법(예: #, *, **, ``` 등), HTML 등 다른 형식은 사용하지 않습니다.
                     - 맨 위에 "AI 설명", "상품 설명" 같은 제목을 넣지 말고, 바로 📝 총평 섹션부터 시작합니다.
                     - 섹션 제목은 아래 5개를 이 순서대로, 한 줄씩 정확히 사용합니다.
                     - 반드시 아래 5가지 항목을 포함하고, 각 항목의 제목 앞에는 지정된 이모지를 붙여주세요.
                     - 절대 다른 설명이나 서두 없이, 오직 5가지 항목과 그 내용만 출력합니다.
                    
                      📝 총평 : 
                      🌡️ 권장 기온대 : 
                      ✨ 상황별 보완 팁 : 
                      👕 체질별 가이드 : 
                      👗 함께 코디하면 좋은 아이템 : 
                    
                     - 각 항목의 세부 내용은 모두 " - "로 시작하는 리스트 형식으로 구조화하여 작성합니다.
                      (예: "- 14–18°C: 실내/외를 오갈 때 ...")
                     - 각 항목의 설명은 2~3개의 리스트 아이템으로 구성해주세요.
                     - 각 리스트 아이템은 1~3문장 정도로 구체적으로 작성합니다.
                     - 각 섹션 사이에는 빈 줄 한 줄만 넣습니다.
                    
                    [출력 항목별 상세 지침 및 형식]
                    1) 📝 총평
                    - 총평 항목의 설명은 1개의 리스트 아이템으로만 구성합니다.
                    - 이 옷의 전반적인 특징, 주요 착용 계절/기온대, 스타일, 전체 인상과 용도(어떤 상황에 잘 맞는지)를 2~3문장으로 간결하게 요약합니다.
                    - 구매 결정을 도와줄 핵심 강점(예: 레이어링 최적, 데일리/출근용으로 무난, 포인트 컬러 등)을 강조합니다.
                    
                    2) 🌡️ 권장 기온대
                    - 섭씨(°C) 기준으로 기온 구간을 나누어 3개의 리스트 항목을 제시합니다.
                    - 각 구간마다 " - ○○-○○°C:" 형식으로 작성합니다.
                      예: "- 14–18°C: 단독 착용으로 쾌적하며, 출퇴근/캠퍼스에 적합합니다."
                    - 각 구간마다 단독 착용 / 이너 추가 / 아우터 레이어링 등 구체적인 착용 방법을 안내합니다.
                    - 기온 수치는 상품의 두께감과 계절 정보를 참고해 현실적인 범위로 설정합니다.
                    - 추가로 마지막 리스트 항목에는 반드시 다음 문장을 포함합니다. (이 항목을 포함하여 총 4개의 리스트 항목임)
                      "- 이 옷에 가장 적절한 최고/최저 기온은 OO°C / XX°C입니다."
                    
                    3) ✨ 상황별 보완 팁
                    - 날씨/환경 상황별로 3개의 리스트 항목을 작성합니다.
                      예: 비/습한 날, 강풍/노상 대기, 난방이 강한 실내, 장시간 실외 활동 등 특정 환경 조건에서의 레이어링/관리 팁을 제공합니다..
                    - 각 항목은 " - 상황: 보완 방법" 형식으로 작성합니다.
                      예: "- 비/습한 날: 니트 위에 가벼운 방수 트렌치나 윈드브레이커를 레이어링하면
                             수분 흡수를 줄이면서 실내외 이동 시 체감 온도를 안정적으로 유지할 수 있습니다."
                    - 아우터/이너/액세서리(머플러, 타이츠, 양말 등)를 활용한 실질적인 해결책을 제시합니다.
                    
                    4) 👕 체질별 가이드
                    - 아래 세 가지 유형에 대해 각각 최소 1개 이상의 리스트 항목을 작성합니다.
                      1. 추위를 많이 타는 편
                      2. 더위를 많이 타는 편
                      3. 피부가 민감하거나 땀이 많은 편
                    - 각 유형마다 어떤 기온에서 어떤 레이어링을 추가해야 하는지, 어떤 소재/핏을 선택하면 좋은지에 대한 최적의 방법을 구체적으로 안내합니다.
                    - 예: "- 추위를 많이 타는 편: 14°C 이하에서는 보온 이너(히트텍/메리노)와 함께 착용하고,
                            바람이 강한 날에는 경량 패딩이나 코트를 더해 체감 온도를 안정적으로 유지하는 것이 좋습니다."
                    
                    5) 👗 함께 코디하면 좋은 아이템
                    - 2개의 리스트 항목으로 기온 구간별로 작성합니다.
                    - 기온대 또는 TPO(출근, 데이트, 캠퍼스, 여행 등)에 따라 3~4개 정도 코디 조합을 제안합니다.
                    - 상·하의, 아우터, 신발, 가방/액세서리를 포함해 "한 벌" 단위로 구체적으로 구성합니다.
                    - 상품의 분류(아우터/상의/하의)에 맞춰 현실적인 조합을 작성합니다.
                      예: 상의 상품이라면 하의/아우터/신발을 중심으로 추천합니다.
                    - 색 조합과 소재 조합(예: 데님 + 화이트 스니커즈, 울 코트 + 가죽 로퍼 등)을 구체적으로 언급합니다.
                    
                    [작성 원칙 및 주의 사항]
                     0. 절대 다른 설명이나 서두 없이, 오직 5가지 항목과 그 내용만 출력합니다.
                     1. 모든 기온 표시는 섭씨(°C) 사용
                     2. 실제 착용 시나리오 기반으로 작성 (출퇴근, 실내외 이동, 야외 활동 등)
                     3. 이모지는 각 항목 제목 앞에만 1개씩 사용
                     4. 제공된 상품 카테고리와 계절 정보를 반드시 고려
                     5. 각 항목의 리스트는 " - " 기호로 시작
                     6. 체질별 가이드는 반드시 3가지만 작성 (추위를 많이 타는 편/보통/더위를 많이 타는 편)
                     7. 상품 정보에 명시되지 않은 소재/디테일(예: 퍼 트리밍, 레더 소재 등)은 이미지에서 매우 명확하게 보이지 않는 이상 단정적으로 적지 않습니다.
                     8. 권장 기온대는 해당 상품을 '주요 아이템'으로 착용했을 때를 기준으로 합니다.
                     9. [출력 예시]
                    
                    
                    [출력 예시]
                    총평
                    해당 상의(도톰한 니트 스웨터)는 간절기부터 초겨울까지 착용하기 좋으며, 대체로 10–18°C 구간에서 가장 활용도가 높다.
                    
                    권장 기온대
                     - 14–18°C: 단독 착용으로 쾌적한 구간으로, 실내외 이동이 잦은 일상/출퇴근에 무난하다.
                     - 10–14°C: 아침·저녁 체감이 떨어지므로 얇은 이너(코튼/히트텍) 추가 또는 라이트 아우터와 레이어링을 권장한다.
                     - 6–10°C: 니트 두께 업 또는 울 코트/경량 패딩과의 레이어링이 필요하며, 보온 이너와 함께 착용 시 체감 안정성이 높다.
                     - 이 옷에 적절한 최고/최저 기온은 18°C/6°C 이다.
                    
                    상황별 보완 팁
                     - 비/습한 날: 니트 위에 발수·방수 외층(라이트 트렌치/윈드브레이커)을 추가하고, 나일론/폴리 혼방 아우터로 수분 흡수를 줄이면 쾌적하다.
                     - 강풍/노상 대기: 바람막이 기능이 있는 트렌치/코치 재킷을 더하고, 목이 시리면 얇은 머플러로 보완하면 체감 온도 하락을 줄일 수 있다.
                     - 난방 강한 실내: 니트는 쉽게 더울 수 있어 민소매/반팔 이너로 체온 조절하고, 필요 시 가디건/숄 대안으로 벗고 입기 편한 레이어를 선택한다.
                    
                    체질별 가이드
                     - 추위를 많이 타는 편: 14°C 이하에서는 보온 이너(히트텍/메리노) + 울·알파카 계열 니트 또는 경량 패딩 베스트를 추가하면 안정적이다.
                     - 더위를 많이 타는 편: 16–20°C에서도 덥게 느낄 수 있어 통기성 좋은 이너와 얇은 게이지 니트, 루즈 핏 선택을 권장하며 실내에서는 쉽게 벗을 수 있게 코디한다.
                     - 피부 민감/가려움: 울 혼방 니트는 간질거림이 있을 수 있어 코튼/모달 이너를 레이어해 마찰과 자극을 줄이는 것이 좋다.
                    
                    함께 코디하면 좋은 아이템
                     - 10–14°C: 트렌치/라이트 블루종 + 코튼 또는 히트텍 이너 + 미디/롱 스커트·슬랙스 조합이 안정적이다.
                     - 6–10°C: 울 코트/경량 패딩 + 타이츠(60–80D)·양말 레이어로 하체 보온을 보강하면 체감이 크게 개선된다.
                    """;

            // 상품 정보/이미지 기반으로 AI 설명 생성
            String productInfo = String.format(
                    "상품명: %s, 카테고리: %s, 성별: %s, 추천 계절: %s",
                    request.getItemName(),
                    request.getCategory(),
                    request.getGender(),
                    String.join(", ", request.getSeasonName() == null ? List.of() : request.getSeasonName())
            );

            // 이미지 리소스 추출 (람다)
            final Resource image =
                    request.getImage() != null ? request.getImage().getResource() : null;

            // OpenAI 옵션 설정
            OpenAiChatOptions options = OpenAiChatOptions.builder()
            .model("gpt-4o-mini")             // OpenAI API 모델 설정
            .temperature(0.6)     // 1.0일수록 창의적, but 느리고 일관성 떨어짐
            //.maxCompletionTokens(1500)        // 응답 길이 제한
            .build();

            // ChatClient 호출 
            String result = chatClient.prompt()
                    .system(systemPrompt)
                    .user(u -> {
                        u.text("다음 상품 정보와 상품 이미지를 보고, 위에서 정의한 형식과 규칙을 정확히 지켜서 설명을 생성해줘. 상품 정보: " + productInfo);
                        if (image != null) {
                            u.media(new MimeType("image","webp"), image);
                        }
                    })
                    .options(options)       // OpenAI 옵션 설정
                    .call()
                    .content();

            // 기온 파싱
            AIResponseDTO.TemperatureInfo tempInfo = parseTemperatureFromDescription(result);
            Integer minTemperature = tempInfo != null ? tempInfo.getMinTemperature() : null;
            Integer maxTemperature = tempInfo != null ? tempInfo.getMaxTemperature() : null;

            return new AIResponseDTO(result, minTemperature, maxTemperature);

        } catch (Exception e) {
            throw new InternalServerException("⚠️ AI 설명 생성 중 오류 발생", e);
        }
    }
}
